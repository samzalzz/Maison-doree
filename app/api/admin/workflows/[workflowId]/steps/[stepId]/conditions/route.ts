...
// ---------------------------------------------------------------------------
// POST /api/admin/workflows/[workflowId]/steps/[stepId]/conditions  (admin only)
// ---------------------------------------------------------------------------
// Creates a new condition attached to a CONDITION step.
// Body: { stepId, field, operator, value }
// ---------------------------------------------------------------------------
...
export const POST = withAdminAuth(async (req: NextRequest, { params }) => {
  try {
    ...
    const condition = await workflowService.createWorkflowCondition({
      ...body,
      stepId,
    })
